...
//                    }
    ...
    public boolean isReplaceableWith(NonStandPowerType<?> newType) {
        return newType == ModVampirism.VAMPIRISM.get();
    }
    ...
    public static void overdriveAttack(LivingEntity user, LivingEntity targetEntity, INonStandPower power, HamonData hamon) {
        ...
        power.clickAction(ModHamon.HAMON_OVERDRIVE.get(), shift, target);
    }
    ...
    public static Set<AbstractHamonSkill> nearbyTeachersSkills(LivingEntity learner) {
        ...
        if (JojoModUtil.entitiesAround(LivingEntity.class, learner, 3D, false, 
...
//        });
    ...
    public static void startLearningHamon(World world, PlayerEntity player, INonStandPower playerPower, LivingEntity teacher, HamonData teacherHamon) {
        if (playerPower.givePower(ModHamon.HAMON.get())) {
        ...
        INonStandPower.getNonStandPowerOptional(dead).ifPresent(power -> {
            power.getTypeSpecificData(ModHamon.HAMON.get()).ifPresent(hamon -> {
                if (hamon.isSkillLearned(ModHamonSkills.CRIMSON_BUBBLE.get())) {
                    ...
                    INonStandPower.getNonStandPowerOptional(player).map(pwr -> pwr.getType() == ModHamon.HAMON.get()).orElse(false))
                            .stream()
                    ...
                    if (closestHamonUser != null) {
                        ...
                        HamonData receiverHamon = INonStandPower.getPlayerNonStandPower(closestHamonUser).getTypeSpecificData(ModHamon.HAMON.get()).get();
                        ...
                        receiverHamon.setHamonStatPoints(BaseHamonSkill.HamonStat.STRENGTH, 
                                receiverHamon.getHamonStrengthPoints() + hamon.getHamonStrengthPoints(), true, false);
                        receiverHamon.setHamonStatPoints(BaseHamonSkill.HamonStat.CONTROL, 
    ...
    public static void coughCoughCough(LivingEntity entity) {
        entity.getCapability(PlayerUtilCapProvider.CAPABILITY).ifPresent(PlayerUtilCap::cough);
    }
}
