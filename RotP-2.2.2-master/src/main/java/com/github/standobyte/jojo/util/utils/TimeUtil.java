    ...
    public static boolean hasTimeStopAbility(LivingEntity entity) {
        return 
                IStandPower.getStandPowerOptional(entity).map(stand -> {
                    return  stand.getType() == ModStandActions.STAND_C_MOON.getStandType() ||
                            stand.getType() == ModStandActions.STAND_MADE_IN_HEAVEN.getStandType() ||
                            Streams.concat(
                            stand.getActions(ActionType.ATTACK).getAll().stream(), 
                            stand.getActions(ActionType.ABILITY).getAll().stream())
                            .anyMatch(action -> allowsToSeeInStoppedTime(action, stand, entity));
                }).orElse(false) 
//                ||
//                INonStandPower.getNonStandPowerOptional(entity).map(power -> {
//                    return Streams.concat(power.getAllActions(ActionType.ATTACK).stream(), power.getAllActions(ActionType.ABILITY).stream())
