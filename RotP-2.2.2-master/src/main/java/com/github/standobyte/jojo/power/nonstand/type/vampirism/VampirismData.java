...
public class VampirismData extends TypeSpecificData {
    ...
    public boolean isActionUnlocked(Action<INonStandPower> action, INonStandPower power) {
        return vampireFullPower || 
                action == ModVampirism.VAMPIRISM_BLOOD_DRAIN.get() || 
                action == ModVampirism.VAMPIRISM_BLOOD_GIFT.get() || 
                vampireHamonUser && action == ModVampirism.VAMPIRISM_HAMON_SUICIDE.get();
    }
...
//        }
    ...
    public static void onEnchantedGoldenAppleEaten(LivingEntity entity) {
        if (!entity.level.isClientSide()) {
            EffectInstance modWeakness = entity.getEffect(ModEffects.VAMPIRE_SUN_WEAKNESS.get());
            if (!(modWeakness != null && modWeakness.getAmplifier() >= 4)) {
                EffectInstance vanillaWeakness = entity.getEffect(Effects.WEAKNESS);
                if (!(vanillaWeakness != null && vanillaWeakness.getAmplifier() >= 4)) {
                    return;
                }
            }
