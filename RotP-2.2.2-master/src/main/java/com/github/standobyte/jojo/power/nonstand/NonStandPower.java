...
public class NonStandPower extends PowerBaseImpl<INonStandPower, NonStandPowerType<?>> implements INonStandPower {
    ...
    @Override
    public boolean clickAction(Action<INonStandPower> action, boolean shift, ActionTarget target) {
        if (!user.level.isClientSide() && type == ModHamon.HAMON.get()) {
            HamonPowerType.coughCoughCough(user);
        }
        return false;
    }
