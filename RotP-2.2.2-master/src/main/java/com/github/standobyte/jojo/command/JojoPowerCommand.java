    ...
    public static void register(CommandDispatcher<CommandSource> dispatcher) { // TODO use the registry
        dispatcher.register(Commands.literal("jojopower").requires(ctx -> ctx.hasPermission(2))
                .then(Commands.literal("give").then(Commands.argument("targets", EntityArgument.players())
                        .then(Commands.literal("hamon").executes(ctx -> giveNonStandPowers(ctx.getSource(), EntityArgument.getPlayers(ctx, "targets"), ModHamon.HAMON.get())))
                        .then(Commands.literal("vampirism").executes(ctx -> giveNonStandPowers(ctx.getSource(), EntityArgument.getPlayers(ctx, "targets"), ModVampirism.VAMPIRISM.get())))))
        ...
        for (ServerPlayerEntity player : targets) {
            ...
            if (power != null) {
                if (!power.hasPower() && power.givePower(powerType)) {
                    i++;
                    power.getTypeSpecificData(ModVampirism.VAMPIRISM.get()).ifPresent(vampirism -> vampirism.setVampireFullPower(true));
                }
