...
public class HamonScreen extends Screen {
    ...
    protected void init() {
        hamon = INonStandPower.getPlayerNonStandPower(minecraft.player)
                .getTypeSpecificData(ModHamon.HAMON.get()).get();
        int i = 0;
        selectableTabs = new HamonTabGui[] {
                new HamonStatsTabGui(minecraft, this, i++, "hamon.stats.tab"),
                new HamonGeneralSkillsTabGui(minecraft, this, i++, "hamon.strength_skills.tab", BaseHamonSkill.HamonStat.STRENGTH),
                new HamonGeneralSkillsTabGui(minecraft, this, i++, "hamon.control_skills.tab", BaseHamonSkill.HamonStat.CONTROL),
    ...
    private void renderInside(MatrixStack matrixStack, int mouseX, int mouseY, int windowX, int windowY, float partialTick) {
        if (selectedTab != null) {
            selectedTab.drawContents(matrixStack, mouseX, mouseY, partialTick, 
