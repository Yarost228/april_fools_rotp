...
public class HamonSkillsManager implements IHamonSkillsManager<AbstractHamonSkill> {
    ...
    private Set<Action<?>> unlockedActions = new HashSet<>();
    
    
    // FIXME !!!!! (technique) set actions which aren't mastered yet
    @Override
    ...
    void syncWithUser(ServerPlayerEntity user, HamonData hamon) {
        for (AbstractHamonSkill skill : getLearnedSkills()) {
            if (containsSkill(skill)) {
                PacketManager.sendToClient(HamonSkillAddPacket.learnNewSkill(skill), user);
            }
        }
