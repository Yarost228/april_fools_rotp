...
//            }
    ...
    private static boolean playerPiercedByArrow(PlayerEntity player, ItemStack stack, World world) {
        ...
        if (!world.isClientSide()) {
            if (!power.hasPower()) {
                ...
                boolean checkTier = true || JojoModConfig.getCommonConfigInstance(false).standTiers.get();
                if (checkTier) {
                ...
                if (stack.getItem() == ModItems.STAND_ARROW_BEETLE.get()) {
                    if (power.getType() == ModStandActions.STAND_GOLD_EXPERIENCE.getStandType()) {
                        power.clear();
                        power.givePower(ModStandActions.STAND_GOLD_EXPERIENCE_REQUIEM.getStandType());
                        power.toggleSummon();
                        PacketManager.sendToClient(new AFPacket(AFPacket.Type.STAND_HUD), (ServerPlayerEntity) player);
                        stack.hurtAndBreak(999999, player, pl -> {});
                        return true;
                    }
                    else if (power.getType() == ModStandActions.STAND_SILVER_CHARIOT.getStandType()) {
                        power.clear();
                        power.givePower(ModStandActions.STAND_CHARIOT_REQUIEM.getStandType());
                        power.toggleSummon();
                        PacketManager.sendToClient(new AFPacket(AFPacket.Type.STAND_HUD), (ServerPlayerEntity) player);
                        stack.hurtAndBreak(999999, player, pl -> {});
                        return true;
    ...
    public void appendHoverText(ItemStack stack, @Nullable World world, List<ITextComponent> tooltip, ITooltipFlag flag) {
        ...
        if (player != null) {
        	...
        	if (true || JojoModConfig.getCommonConfigInstance(true).standTiers.get()) {
