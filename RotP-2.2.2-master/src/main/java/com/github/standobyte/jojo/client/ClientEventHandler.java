...
public class ClientEventHandler {
    ...
    public <T extends LivingEntity, M extends EntityModel<T>> void onRenderLiving2(RenderLivingEvent.Pre<T, M> event) {
        ...
        INonStandPower.getNonStandPowerOptional(entity).ifPresent(power -> {
            ...
            if (heldAction instanceof HamonTornadoOverdrive) {
                if (((HamonTornadoOverdrive) heldAction).isHorizontal()) {
                    event.getMatrixStack().mulPose(Vector3f.YP.rotationDegrees(180 - 
                            entity.getCapability(LivingUtilCapProvider.CAPABILITY).map(LivingUtilCap::getYRotOnLastSendoWaveKick)
                            .orElse(MathHelper.lerp(event.getPartialRenderTick(), entity.yRotO, entity.yRot))));
                    event.getMatrixStack().mulPose(Vector3f.XP.rotationDegrees(90));
                }
                event.getMatrixStack().mulPose(Vector3f.YP.rotation((power.getHeldActionTicks() + event.getPartialRenderTick()) * 2F % 360F));
                
            }
            if (power.isActionOnCooldown(ModHamon.HAMON_ZOOM_PUNCH.get())) {
    ...
    public <T extends LivingEntity, M extends EntityModel<T>> void onRenderNameplate(RenderNameplateEvent event) {
        ...
        if (entity instanceof LivingEntity) {
            INonStandPower.getNonStandPowerOptional((LivingEntity) entity).ifPresent(power -> {
                if (power.getHeldAction(true) == ModHamon.ZEPPELI_TORNADO_OVERDRIVE.get()) {
            ...
            if (entity instanceof LivingEntity) {
                ...
                INonStandPower.getNonStandPowerOptional(livingEntity).ifPresent(power -> {
                    if (power.isActionOnCooldown(ModHamon.HAMON_ZOOM_PUNCH.get())) {
                        ...
                        if (hud.getSelectedAction(ActionType.ATTACK) == ModHamon.JONATHAN_OVERDRIVE_BARRAGE.get()
    ...
    public void addTooltipLines(ItemTooltipEvent event) {
        ...
        if (event.getItemStack().getItem() instanceof EnchantedBookItem && !ModList.get().isLoaded("enchdesc")) {
            EnchantedBookItem.getEnchantments(event.getItemStack()).forEach(nbt -> {
                if (nbt.getId() == JojoModUtil.getNbtId(CompoundNBT.class)) {
