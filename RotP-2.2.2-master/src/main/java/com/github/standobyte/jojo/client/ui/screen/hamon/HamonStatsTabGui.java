...
public class HamonStatsTabGui extends HamonTabGui {
    ...
    protected void drawActualContents(MatrixStack matrixStack, int mouseX, int mouseY) {
        ...
        if (hamonStrengthLimited = level < HamonData.MAX_STAT_LEVEL && level >= (int) breathingTechnique + HamonData.MIN_BREATHING_EXCEED) {
        ...
        if (hamonControlLimited = level < HamonData.MAX_STAT_LEVEL && level >= (int) breathingTechnique + HamonData.MIN_BREATHING_EXCEED) {
    ...
    void drawTab(MatrixStack matrixStack, int windowX, int windowY, boolean isSelected) {
        ...
        int iconY = windowY + getTabY(tabIndex) + 6;
    ...
    void drawToolTips(MatrixStack matrixStack, int mouseX, int mouseY, int windowPosX, int windowPosY) {
        ...
        if (bonus > 0 && 
        ...
        else if (screen.hamon.allExerceisesCompleted() && mouseX >= 196 && mouseX < 204 && mouseY >= exercisesAvgY && mouseY < exercisesAvgY + 8) {
            screen.renderTooltip(matrixStack, minecraft.font.split(new TranslationTextComponent(
                    "hamon.exercise.full_completion_buff", PERCENTAGE_FORMAT.format((HamonData.ALL_EXERCISES_EFFICIENCY_MULTIPLIER - 1F) * 100F)), 100), mouseX, mouseY);
        }
            ...
            for (HamonData.Exercise exercise : HamonData.Exercise.values()) {
                ...
                if (mouseX >= x && mouseX < x + 8 && mouseY >= y && mouseY < y + 8 && screen.hamon.isExerciseComplete(exercise)) {
