...
public class TrHamonParticlesPacket {
    ...
    private static void writeParticle(IParticleData particleData, PacketBuffer buf) {
        buf.writeInt(Registry.PARTICLE_TYPE.getId(particleData.getType()));
        particleData.writeToNetwork(buf);
    }
    
    private static Optional<IParticleData> readParticle(PacketBuffer buf) {
    ...
    private static <T extends IParticleData> T readParticle(PacketBuffer buf, ParticleType<T> type) {
