...
public class RockPaperScissorsGame {
    ...
    private void boy2Man(LivingEntity roundWinner, LivingEntity roundLoser, int round) {
        ...
        if (loserStand.hasPower() && winnerStand.getType() == ModStands.BOY_II_MAN.getStandType()) {
            // FIXME (BIIM)
        ...
        else if (loserStand.getType() == ModStands.BOY_II_MAN.getStandType() && round == ROUNDS_TO_WIN) {
    ...
    private static void initCheat() {
        if (!cheatInitialized) {
            ...
            CHEATS.put(PowerClassification.NON_STAND, Util.make(Maps.newHashMap(), map -> {
                map.put(ModHamon.HAMON.get(), (game, player, world) -> {
                    if (!world.isClientSide()) {
                        ServerWorld serverWorld = (ServerWorld) world;
                ...
                map.put(ModVampirism.VAMPIRISM.get(), (game, player, world) -> {
                    if (!world.isClientSide()) {
                        ServerWorld serverWorld = (ServerWorld) world;
                        RockPaperScissorsPlayerData opponent = game.getOpponent(player);
