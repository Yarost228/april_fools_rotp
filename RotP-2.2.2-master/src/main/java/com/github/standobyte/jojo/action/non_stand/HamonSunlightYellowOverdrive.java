...
// FIXME ! (hamon) sunlight yellow overdrive
/*
 * charge visuals
 */
public class HamonSunlightYellowOverdrive extends HamonAction {
    ...
    public int getHoldDurationToFire(INonStandPower power) {
        ...
        int ret = MathHelper.ceil(ModHamon.HAMON_OVERDRIVE.get().getEnergyCost(power) * maxTicks / power.getMaxEnergy());
    ...
    public int getHoldDurationMax(INonStandPower power) {
        ...
        if (ticks == 0) return 0;
        float control = power.getTypeSpecificData(ModHamon.HAMON.get()).map(HamonData::getHamonControlLevelRatio).orElse(0F);
