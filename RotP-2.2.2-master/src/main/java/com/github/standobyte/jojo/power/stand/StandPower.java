...
//        }).orElse(1F);
    ...
    private void tickStamina() {
        if (usesStamina()) {
            prevTickStamina = getStamina();
            addStamina(getStaminaTickGain(), false);
        }
    }
    
    @Override
    public float getPrevTickStamina() {
        return prevTickStamina;
    }
...
//            }).orElse(1F);
    ...
    public float getResolveDmgReduction() {
        ...
        power.getType() == ModVampirism.VAMPIRISM.get()).orElse(false)) {
            return 0;
        }
