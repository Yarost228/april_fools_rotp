    ...
    public void remove() {
        ...
        if (tickCount < ticksLifespan()) {
            INonStandPower.getNonStandPowerOptional(getOwner()).ifPresent(power -> {
                power.updateCooldownTimer(ModHamon.HAMON_ZOOM_PUNCH.get(), 0, ModHamon.HAMON_ZOOM_PUNCH.get().getCooldownTechnical(null));
            });
        }
    }
    ...
    private Optional<HamonData> getUserHamon() {
        if (!hamon.isPresent()) {
            hamon = INonStandPower.getNonStandPowerOptional(getOwner())
                    .map(power -> power.getTypeSpecificData(ModHamon.HAMON.get())).orElse(hamon);
        }
        return hamon;
    }
