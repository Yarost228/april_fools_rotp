...
public class HamonBubbleBarrierEntity extends ModdedProjectileEntity {
    ...
    public HamonBubbleBarrierEntity(World world, LivingEntity shooter, INonStandPower power) {
        ...
        this.power = power;
        barrierMaxTicks = (int) (100F * power.getTypeSpecificData(ModHamon.HAMON.get())
    ...
    public void tick() {
        ...
        if (!level.isClientSide()) { 
            if (barrier && (barrierTicks++ >= barrierMaxTicks || !isVehicle()) || power == null) {
                ...
                if (power.getHeldAction() != ModHamon.CAESAR_BUBBLE_BARRIER.get()) {
                    remove();
                }
                else if (power.getHeldActionTicks() >= ModHamon.CAESAR_BUBBLE_BARRIER.get().getHoldDurationToFire(power) - 1) {
            ...
            if (owner != null) {
                INonStandPower.getNonStandPowerOptional(owner).ifPresent(power -> {
                    power.getTypeSpecificData(ModHamon.HAMON.get()).ifPresent(hamon -> {
                        hamon.hamonPointsFromAction(HamonStat.STRENGTH, ModHamon.CAESAR_BUBBLE_BARRIER.get().getHeldTickEnergyCost(power) / 4F);
                    });
                });
            }
        }
    ...
    public float getSize(float partialTick) {
        return Math.min((tickCount + partialTick) / (float) ModHamon.CAESAR_BUBBLE_BARRIER.get().getHoldDurationToFire(null), 1);
    }
